...
const INIT_REPORTS = [{
  id: "r1", folio: "MNT-001", clientId: 1, assignedTo: 2,
...
function NewReportModal({ onClose, onSave, clients, users, currentUser }) {
  const [form, setForm] = useState({ clientId: "", title: "", description: "", date: today(), assignedTo: currentUser.id });
  ...
  function save() {
    ...
    onSave({ id: genId(), folio, ...form, clientId: form.clientId, assignedTo: parseInt(form.assignedTo), status: "borrador", photos, findings, budget: { items: [], subtotal: 0, iva: 0, total: 0, advancePct: 50, advancePaid: false, finalPaid: false }, schedule: [], timeline: [{ date: today(), event: "Reporte creado", user: currentUser.name }] });
  }
        ...
        {tab === "overview" && (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
              {[["Cliente", client?.name, client?.email], ["Asignado a", assigned?.name, assigned?.role], ["Inspección", fmtDate(report.date), `Folio: ${report.folio}`]].map(([l, v, s]) => (
                <div key={l} style={{ background: "#1f2937", borderRadius: 10, padding: 12 }}>
                  ...
                  <div style={{ color: "#f9fafb", fontWeight: 700 }}>{v}</div>
